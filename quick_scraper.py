"""
quick_scraper.py - Quick Start Web Scraping for Heart Disease Model
Simple interface to gather medical data and enhance the prediction model
"""
...
def main():
    """Quick start interface for medical data scraping and processing"""
    
    print("=" * 70)
    print("HEART DISEASE MODEL ENHANCEMENT - QUICK START")
    print("=" * 70)
    ...
    # Research papers
    try:
        max_papers = int(input("Number of research papers to scrape (default 50): ") or "50")
    except ValueError:
        max_papers = 50
    
    # Clinical trials
    try:
        max_trials = int(input("Number of clinical trials to scrape (default 30): ") or "30")
    except ValueError:
        max_trials = 30
    
    # Custom search query
    custom_query = input("Custom search query (press Enter for default): ").strip()
    if not custom_query:
        custom_query = "heart disease prediction machine learning biomarkers"
    
    print(f"\nStarting data collection with:")
